...
TSP::TSP (const char *tsp_file_name){
	...
	// Read lines
	if (fscanf(tsp_file,"%s", buf))
		//printf("\n File successfully scanned %s ... \n", tsp_file_name);
	while ( strcmp("NODE_COORD_SECTION", buf) != 0 ) {
		if ( strcmp("NAME", buf) == 0 ) {
			...
			if (fgets(buf, LINE_BUF_LEN, tsp_file))
			TRACE ( printf("%s", buf); );
			buf[0]=0;
		}
		else if ( strcmp("COMMENT:", buf) == 0 ){
	...
	if( (nodeptr = (point *) malloc(sizeof(struct point) * n)) == NULL )
		...
		for ( i = 0 ; i < n ; i++ ) {
			if(fscanf(tsp_file,"%ld %lf %lf", &j, &nodeptr[i].x, &nodeptr[i].y ))
				printf(" ");
		}
	}
