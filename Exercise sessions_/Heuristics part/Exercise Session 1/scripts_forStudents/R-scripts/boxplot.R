...
do.boxplot <- function (data.matrix, data.labels=colnames(data.matrix), plot.mar=c(4,14,4,2), plot.title="Sample acotsp", plot.yaxis.label="Objective function value", output="aco_TSP_ants_bxp.png", plot.axis.style=1, plot.size.axis=3, plot.size.title=3, plot.text.yaxis=3, vertical.x.axis=1, colors=rep(0, ncol(data.matrix))){
  ...
  axis(1, at=c(1:length(data.labels)), labels=data.labels, line=plot.mar[1] - 3, tick=FALSE, las=vertical.x.axis, cex.axis=plot.size.axis)
