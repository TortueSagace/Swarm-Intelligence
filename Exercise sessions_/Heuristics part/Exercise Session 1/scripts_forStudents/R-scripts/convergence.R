do.lines.plot <- function(data.list=NULL, test.names=names(data.list), plot.mar=c(6,16,4,3), plot.title="Sample acotsp", plot.yaxis.label="Objective function value",plot.xaxis.label="Tours", output="aco_TSP_ants_convergence.png", plot.axis.style=1, plot.size.axis=3, plot.size.title=3){
  ...
  legend("topright", "(x,y)", lty=line.type, lwd=rep(2,length(line.type)), col=line.col, legend=names(data.list), xjust=1, yjust=1, seg.len=3, cex=3, bg="white")
  axis(2)
