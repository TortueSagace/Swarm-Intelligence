...
void initialize() {
	...
	for(int i=0;i<n;i++)
		global_best.x[i] = 0;
	global_best.eval=DBL_MAX;              //initialized with a large value
...
/*Move the swarm */
void moveSwarm() {
	double prev_eval=global_best.eval;    //iteration t-1 best value
	for (int i=0;i<n_particles;i++){
		swarm[i].move();                 //particles move applying the rules for new velocity and new position
	...
	if(prev_eval > global_best.eval)
		cout << "\tNew best solution: " << global_best.eval << " found at iteration " << iterations << endl;
}
